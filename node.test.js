const { generateLogo, shapes } = require('./node');

test('generateLogo', () => {
    generateLogo('ABC', 'red', shapes[0], '#00ff00');
    const svgData = fs.readFileSync('logo.svg', 'utf8');
    expect(svgData).toContain('text x="150" y="100" fill="red">ABC</text>');
    expect(svgData).toContain('circle cx="150" cy="100" r="50" fill="00ff00"');
});

test('generateLogo with triangle', () => {
    generateLogo('ABC', 'red', shapes[0], 'ff0000');
    const svgData = fs.readFileSync('logo.svg', 'utf8');
    expect(svgData).toContain('text x="150" y="100" fill="blue">DEF</text>');
